...
import { applyOpenAPIDecorator } from '../openapi/decorators';
import { getResponses, IRoute } from '../openapi/common';
...
export function getOperation(
  ...
  const operation: oa.OperationObject = {
    ...
    summary: options?.defaults?.summary || getSummary(route),
    tags: getTags(route),
  };
 ...
 */
export function getOperationId(route: IRoute, withControllerName?: boolean): string {
  if (!withControllerName) return route.action.method;
...
export function getPathParams(route: IRoute): oa.ParameterObject[] {
  ...
  return tokens
    ...
    .map((_token) => {
      const token = _token as pathToRegexp.Key;
      const name = token.name + '';
      const param: oa.ParameterObject = {
        in: 'path',
        name,
        required: true, // Fix - path params must be true as part of request
        schema: {
          type: 'string',
        },
      };
