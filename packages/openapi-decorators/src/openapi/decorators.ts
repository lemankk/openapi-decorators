...
import { getContentType, getStatusCode, IRoute } from './common';
...
export function getOpenAPIMetadata(target: object, key?: string): OpenAPIParam[] {
  return (
    (key ? Reflect.getMetadata(OPEN_API_KEY, target.constructor, key) : Reflect.getMetadata(OPEN_API_KEY, target)) || []
  );
}
